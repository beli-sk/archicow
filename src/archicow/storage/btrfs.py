...
class BtrfsStorage(BaseStorage):
    """Incremental storage using Btrfs snapshots.
    ...
    def new_target(self, type_, path):
        ...
        if type_ == target_type.DIR:
            ...
            if latest is None:
                logger.debug('%s: creating snapshot %s',
                        self.__class__.__name__, target_path_ts)
                target_path_ts.parent.mkdir(parents=True)
                args = [self.btrfs_path, 'subvolume', 'create', target_path_ts]
                subprocess.check_call(args, shell=False)
            else:
                logger.debug('%s: creating snapshot %s from %s',
